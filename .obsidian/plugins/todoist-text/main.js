  ...
  "node_modules/axios/package.json"(exports, module2) {
    module2.exports = {
      _args: [
        [
          "axios@0.21.4",
          "/home/runner/work/obsidian-todoist-text/obsidian-todoist-text"
        ]
      ],
      _from: "axios@0.21.4",
      _id: "axios@0.21.4",
      ...
      _spec: "0.21.4",
      _where: "/home/runner/work/obsidian-todoist-text/obsidian-todoist-text",
      author: {
        name: "Matt Zabriskie"
      },
      browser: {
  ...
  "node_modules/axios-case-converter/lib/decorators.js"(exports) {
    ...
    var applyCaseOptions = function applyCaseOptions2(fn2, defaultOptions) {
      return function(input, options) {
        return fn2(input, _objectSpread(_objectSpread({}, defaultOptions), options));
      };
    };
    exports.applyCaseOptions = applyCaseOptions;
    var preserveSpecificKeys = function preserveSpecificKeys2(fn2, keys) {
...
var require_transformers = __commonJS({
  "node_modules/axios-case-converter/lib/transformers.js"(exports) {
    ...
    var transformObjectUsingCallbackRecursive = function transformObjectUsingCallbackRecursive2(data, fn2, overwrite) {
    ...
    var transformObjectUsingCallback = function transformObjectUsingCallback2(data, fn2, options) {
      fn2 = (0, _decorators.applyCaseOptions)(fn2, _objectSpread({
        stripRegexp: /[^A-Z0-9[\]]+/gi
      }, options === null || options === void 0 ? void 0 : options.caseOptions));
      if (options !== null && options !== void 0 && options.preservedKeys) {
        fn2 = (0, _decorators.preserveSpecificKeys)(fn2, options.preservedKeys);
      }
  ...
  return __async(this, null, function* () {
    ...
    for (const keywordToQuery of settings.keywordToTodoistQuery) {
      if (keywordToQuery.keyword.length > 1 && keywordToQuery.todoistQuery.length > 1 && fileContents.contains(keywordToQuery.keyword)) {
        ...
        const formattedTodos = yield getServerData(keywordToQuery.todoistQuery, settings.authToken);
  ...
  return __async(this, null, function* () {
    try {
      ...
      if (tryingToClose) {
        ...
        new import_obsidian.Notice(`Todoist Text: Closed "${serverTaskName}" on Todoist`);
      }
      if (tryingToReOpen) {
        yield api.reopenTask(taskId);
        new import_obsidian.Notice(`Todoist Text: Re-opened "${serverTaskName}" on Todoist`);
      }
    } catch (e2) {
...
function getServerData(todoistQuery, authToken) {
  return __async(this, null, function* () {
    const api = new import_todoist_api_typescript.TodoistApi(authToken);
    let tasks;
    try {
      tasks = yield api.getTasks({ filter: todoistQuery });
    } catch (e) {
      let errorMsg;
...
var TodoistPluginSettingTab = class extends import_obsidian4.PluginSettingTab {
  ...
  addExcludedDirectoriesSetting(containerEl) {
    ...
    excludedFolderDescription.append("If you use template files (e.g. for daily notes) and you want to use a keyword in that template file, this plugin would replace the keyword in your template file with Todos immediately, rendering the template useless.", excludedFolderDescription.createEl("br"), "To prevent this, exclude the folder containing your template file.");
    new import_obsidian4.Setting(this.containerEl).setDesc(excludedFolderDescription);
    this.plugin.settings.excludedDirectories.forEach((dir, index) => {
      new import_obsidian4.Setting(this.containerEl).setName("Excluded folder").addSearch((cb) => {
        ...
        eb.setIcon("cross").setTooltip("Delete").onClick(() => __async(this, null, function* () {
          this.plugin.settings.excludedDirectories.splice(index, 1);
          yield this.plugin.saveSettings();
          yield this.display();
        }));
      });
  ...
  addKeywordTodoistQuerySetting(containerEl) {
    ...
    filterDescription.append("This plugin will find the specified keyword in a currently open file and replace the keyword with your Todos. Your Todos will be pulled from Todoist based on the specified ", containerEl.createEl("a", null, (link) => {
      link.href = "https://todoist.com/help/articles/introduction-to-filters";
      link.innerText = "filter definition.";
    }), containerEl.createEl("br"), "Each keyword you use should be unique.");
    new import_obsidian4.Setting(containerEl).setDesc(filterDescription);
    this.plugin.settings.keywordToTodoistQuery.forEach((keywordToTodoistQuery, index) => {
      const div = this.containerEl.createEl("div");
      div.addClass("todoist-setting-div");
      new import_obsidian4.Setting(containerEl).addText((text) => text.setPlaceholder("@@TODOIST_KEYWORD@@").setValue(this.plugin.settings.keywordToTodoistQuery[index].keyword).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.keywordToTodoistQuery[index].keyword = value;
        yield this.plugin.saveSettings();
      })).inputEl.addClass("todoist-query-setting")).addText((text) => text.setPlaceholder("today|overdue").setValue(this.plugin.settings.keywordToTodoistQuery[index].todoistQuery).onChange((value) => __async(this, null, function* () {
        this.plugin.settings.keywordToTodoistQuery[index].todoistQuery = value;
        yield this.plugin.saveSettings();
      })).inputEl.addClass("todoist-query-setting")).addExtraButton((eb) => {
        eb.setIcon("cross").setTooltip("Delete").onClick(() => __async(this, null, function* () {
          this.plugin.settings.keywordToTodoistQuery.splice(index, 1);
          yield this.plugin.saveSettings();
          yield this.display();
        }));
      });
